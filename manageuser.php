...
} elseif ($action == "edit")
{
    ...
    if (!empty($_FILES['userfile']['name']))
    {
        ...
        if ($user->edit($userid, $name, $email, $company, $zip, $gender, $turl, $address1, $address2, $state, $country, "", $locale, $avatar, $admin))
        {
    ...
    {
        if ($user->edit($userid, $name, $email, $company, $zip, $gender, $turl, $address1, $address2, $state, $country, "", $locale, "", $admin))
        {
            if (isset($oldpass) and isset($newpass) and isset($repeatpass))
            {
                $user->editpass($userid, $oldpass, $newpass, $repeatpass);
...
} elseif ($action == "onlinelist")
{
    ...
    if (!empty($onlinelist))
    {
        ...
        {
            echo "<ul><li><a class=\"user\" href = \"manageuser.php?action=profile&id=$online[ID]\">$online[name]</a>";
            if ($online['ID'] != $userid)
            {
