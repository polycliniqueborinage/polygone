...
} elseif ($action == "adduser")
{
    ...
    $newid = $user->add($name, $email, 0, $pass, $admin, $sysloc, $tags);
    if ($newid)
    {
    ...
    else
    {
        ...
        $template->assign("errortext", "The email address $email or the username $name already exists in the Userdatabase.<br>$goback");
        $template->display("error.tpl");
    }
...
} elseif ($action == "edituser")
{
    ...
    if (!empty($_FILES['userfile']['name']))
    {
        ...
        $datei_final = "./files/avatar/" . $fname;
        ...
        if ($user->edit($id, $name, $email, $company, $zip, $gender, $turl, $address1, $address2, $state, $country, $tags, $locale, $avatar, $isadmin))
        {
    ...
    {
        if ($user->edit($id, $name, $email, $company, $zip, $gender, $turl, $address1, $address2, $state, $country, $tags, $locale, "", $isadmin))
        {
            if (!empty($newpass) and !empty($repeatpass))
            {
                $user->admin_editpass($id, $newpass, $repeatpass);
...
{
    $add = $project->add($name, $desc, $end, 0);
    if ($add)
    {
...
{
    if ($theset->editSettings($name, $subtitle, $locale, $timezone, $templ, $favicon))
    {
    	$_SESSION["userlocale"] = $locale;
    	...
    	foreach($users as $theuser)
    	{
    		...
    		$user->edit($theuser["ID"],$theuser["name"],$theuser["email"],0,$theuser["zip"],$theuser["gender"],$theuser["url"],$theuser["adress"],$theuser["adress2"],$theuser["state"],$theuser["country"],$theuser["tags"],$locale,"",$theuser["admin"]);
    	}
