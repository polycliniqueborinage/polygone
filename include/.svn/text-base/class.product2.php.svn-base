...
class product
{
    ...
    function get($id) {
    	...
    	$sql = "SELECT p.ID as ID, p.name as name, p.unit as unit, p.size as size, p.dose as dose,p.stock as stock, p.sail_price_htva as sail_price_htva, p.tva as tva, p.sail_price as sail_price, p.description as description, ROUND(SUM( pf.quantity * pf.type ),2) as current_stock, ROUND(SUM( pf.quantity * pf.type * p.sail_price ),2) as total_price FROM product p, product_flow pf WHERE p.ID =$id AND p.ID = pf.product_ID GROUP BY p.ID";
        ...
        $sql = "SELECT lot_number as lot_number FROM product_flow WHERE product_ID =$id ORDER BY date DESC";
        $sel = mysql_query($sql);
    ...
    function modulesearch($id,$value,$limit,$unit1,$unit2,$unit3,$unit4,$unit5) {
    	...
    	if ($id!='undefined' && $id!='undefined' && $id!= null)
    		$sql = "SELECT p.ID as ID, p.name as name, p.unit as unit, p.size as size, p.dose as dose,p.stock as stock, p.description as description, p.sail_price_htva as sail_price_htva, p.tva as tva, p.sail_price as sail_price, ROUND(SUM( pf.quantity * pf.type ),2) as current_stock, ROUND(SUM( pf.quantity * pf.type * p.sail_price ),2) as total_price FROM product p, product_flow pf WHERE p.ID =$id AND p.ID = pf.product_ID GROUP BY p.ID";
		else
    		$sql = "SELECT p.ID as ID, p.name as name, REPLACE(REPLACE(REPLACE(REPLACE(REPLACE( p.unit, '4', '$unit4' ),'3','$unit3'),'2','$unit2'),'1','$unit1'),'5','$unit5') as unit, p.size as size, p.dose as dose,p.stock as stock, p.description as description, p.sail_price_htva as sail_price_htva, p.tva as tva, p.sail_price as sail_price, ROUND(SUM( pf.quantity * pf.type ),2) as current_stock, ROUND(SUM( pf.quantity * pf.type * p.sail_price ),2) as total_price FROM product p, product_flow pf WHERE lower(name) regexp '$value' AND p.ID = pf.product_ID GROUP BY p.ID LIMIT $limit";
		//echo $sql;
    ...
    function autocomplete($value,$id) {
	    ...
	    if ($id!='undefined' && $id!='undefined' && $id!= null)
	    	...
	    	$sql = "SELECT p.ID as ID, p.name as name, p.unit as unit, p.size as size, p.dose as dose,p.stock as stock, p.sail_price_htva as sail_price_htva, p.tva as tva, p.sail_price as sail_price, p.description as description, ROUND(SUM( pf.quantity * pf.type ),2) as current_stock, ROUND(SUM( pf.quantity * pf.type * p.sail_price ),2) as total_price FROM product p, product_flow pf WHERE lower(p.name) regexp '$value' AND p.ID = pf.product_ID GROUP BY p.ID";
	...
	function getAll() {
        ...
        $sql = "SELECT DISTINCT p.ID as ID, p.name as name, p.unit as unit, p.size as size, p.dose as dose,p.stock as stock, p.sail_price_htva as sail_price_htva, p.tva as tva, p.sail_price as sail_price, p.description as description, SUM( pf.quantity * pf.type ) as current_stock,  SUM( pf.quantity * pf.type * p.sail_price ) as total_price FROM product p, product_flow pf WHERE p.ID = pf.product_ID GROUP BY p.ID";
