...
class FPDF
{
    ...
    function _putinfo()
    {
        $this->_out('/Producer ' . $this->_textstring("Collabtive"));
        if (!empty($this->title))
...
class PDF_table extends FPDF
{
    ...
    // Better table
    function FancyTable($head, $data, $tsize, $totaltime = 0,$title = "timetable")
    {
        $this->SetFont('Arial', 'B', $tsize + 2);
        $this->Cell(80);
        $this->Cell(30, 10, $title, 0, 0, 'C');
        $this->Ln(10);
        // Colors, line width and bold font
        $this->SetFillColor("#d9dee8");
        $this->SetTextColor("black");
        $this->SetDrawColor("#B1B7C3");
        $this->SetLineWidth(.3);
        $this->SetFont('Arial', 'B', $tsize + 2);
        // Header
        $w = $head[1];
        ...
        if ($totaltime > 0)
        {
            $this->Cell(array_sum($w), 6, $totaltime, "LR", 0, "R", $fill);
            $this->Ln();
        }
...
class PDF_plog extends FPDF
{
    ...
    // Colored table
    function FancyTable($head, $data, $tsize, $pname)
    {
        $this->SetFont('Arial', 'B', $tsize + 4);
        $this->Cell(80);
        $this->Cell(30, 10, $pname, 0, 0, 'C');
        $this->Ln(10);
        // Colors, line width and bold font
        $this->SetFillColor("#d9dee8");
        $this->SetTextColor("black");
        ...
        foreach($data as $row)
        {
            // $row[1] = $this->WordWrap($row[1],10 );
            $this->Cell($w[0], 6, $row[0], 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 6, $row[1], 'LR', 0, 'L', $fill);
            if ($row[5] == 3)
            {
                $this->SetTextColor("#c62424");
            } elseif ($row[5] == 1 or $row[5] == 5)
            {
                $this->SetTextColor("#377814");
            } elseif ($row[5] == 2 or $row[5] == 4)
            {
                $this->SetTextColor("#a68a03");
            }
            $this->Cell($w[2], 6, $row[2], 'LR', 0, 'C', $fill);
...
// //////////////////////////////////
class pdfhtml extends PDF_Diag
{
        ...
        foreach($a as $i => $e)
        {
            if ($i % 2 == 0)
            {
                ...
                if ($this->HREF)
                    ...
                    $this->Write(10, stripslashes(txtentities($e)));
            }
            else
            {
                // Tag
