...
SUGAR.grid = function() {
	...
	return {
		...
		pasteRow: function()
		{
			...
			if (selectedRows.length == 0)
				insertAtRow = table.rows.length - 1;
			else
			...
			while (copyRows.length > 0) {
				...
				//document.getElementById("percent_complete_" + mappedRow).value = document.getElementById("percent_complete_" + copyRows[copyRows.length - 1]).value;
				...
				//document.getElementById("resource_" + mappedRow).value = document.getElementById("resource_" + copyRows[copyRows.length - 1]).value;
			...
			while (cutRows.length > 0) {
				...
				SUGAR.grid.insertRow(insertAtRow);
				var mappedRow = SUGAR.grid.getMappedRow(insertAtRow);
				//document.getElementById("percent_complete_" + mappedRow).value = cutRows[0].percent_complete;
				document.getElementById("duration_" + mappedRow).value =
					cutRows[0].duration;
				document.getElementById("duration_unit_" + mappedRow).value =
				...
				document.getElementById("date_finish_" + mappedRow).value = cutRows[0].date_finish;
				//document.getElementById("resource_" + mappedRow).value = cutRows[0].resource;
		...
		insertRow: function(atRow)
		{
			...
			cell.innerHTML =
				...
				"<input type='hidden' id='save_in_progress_" + numRows + "'>" +
				"<input type='hidden' name='is_milestone_" + numRows + "' id='is_milestone_" + numRows + "'>" ;
			cell.setAttribute('class', 'PM CS CD color1');
			
			// Stage
			cell = row.insertCell(1);
			cell.innerHTML = "<select name='stage_" + numRows + "' id='stage_" + numRows + "'" +	" onchange='javascript:SUGAR.grid.save(" + numRows + ");'>" +
							 "<option value='JS'>JS</option><option value='PP'>PP</option><option value='PR'>PR</option><option value='CE'>CE</option><option value='PO'>PO</option><option value='JW'>JW</option></select>";
			cell.setAttribute('class', 'PM color1');
			
			
			// Description
			cell = row.insertCell(2);
			cell.ondblclick = function(){current_id=numRows;SUGAR.grid.toggle("description_" + numRows + "_div", "description_" + numRows, "description_" + numRows + "_divlink");};
			cell.innerHTML =
				"<div id='description_" + numRows + "_div' style='display:none;'> " +
				"<input class='description' type=text style='border:0'  size=40 name=description_" + numRows + "  id=description_" + numRows +
				" onkeyup='javascript:SUGAR.grid.save(" + numRows +  ");' onblur='SUGAR.grid.blurEvent(" + numRows + ", \"description_" + numRows + "_div\",\"description_" + numRows + "\", \"description_" + numRows + "_divlink\");' " +
			...
			cell.innerHTML =
					...
					" onChange='SUGAR.grid.updateResourceFullNameAndType("+ numRows +");" + 
								"SUGAR.grid.updateResourceUsed(" + numRows + ");" +
								"SUGAR.grid.calculateEndDate(document.getElementById(\"date_start_"+ numRows + "\").value, document.getElementById(\"duration_" + numRows + "\").value, "+ numRows + "); " +
								"SUGAR.grid.calculateDatesAfterAddingPredecessors(" + numRows + ");" +
								"SUGAR.grid.updateAllDependentsAfterDateChanges(" + numRows + ");" +
								"SUGAR.gantt.changeTask(" + numRows +");SUGAR.grid.save(" + numRows +  ");'>" +
					resourceOptionsString + "</select>";
			cell.setAttribute('class', 'PM CS CD color1');

			// Resource_type 
			cell = row.insertCell(4);
			cell.innerHTML = "<input type='text' class='resource_used small' name='resource_used_" + numRows +  "' id='resource_used_" + numRows +  "' value='' onclick='javascript:current_id=" + numRows +  "' onkeyup='javascript:SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CS CD color1');
			cell.setAttribute('style', 'display:none');
			
			//Language
			cell = row.insertCell(5);
			cell.innerHTML = "<input type='text' class='source_language small' name='source_language_" + numRows +  "' id='source_language_" + numRows +  "' value='' onclick='javascript:current_id=" + numRows +  "' onkeyup='javascript:SUGAR.grid.save();'>";
			cell.setAttribute('class', 'PM CS CD color1');
			
			//Language
			cell = row.insertCell(6);
			cell.innerHTML = "<input type='text' class='target_language small' name='target_language_" + numRows +  "' id='target_language_" + numRows +  "' value='' onclick='javascript:current_id=" + numRows +  "' onkeyup='javascript:SUGAR.grid.save();'>";
			cell.setAttribute('class', 'PM CS CD color1');

			//Worktype
			cell = row.insertCell(7);
			cell.innerHTML = "<input type='text' class='worktype small' name='worktype_" + numRows +  "' id='worktype_" + numRows +  "' value='' onclick='javascript:current_id=" + numRows +  "' onkeyup='javascript:SUGAR.grid.save();'>";
			cell.setAttribute('class', 'PM CS CD color1');
			
			// complete
			cell = row.insertCell(8);
			cell.innerHTML = "<select name='status_" + numRows + "' id='status_" + numRows + "'" + " onchange=\"javascript:$('#percent_complete_"+ numRows + "').val(this.value);if(this.value!=100) $('#project_task_row_"+ numRows + "').removeClass('complete_100');if(this.value==100) $('#project_task_row_"+ numRows + "').addClass('complete_100');if(this.value==100 && $('#resource_used_"+ numRows + "').val()!='EXT') $('#final_effort_"+ numRows + "').val($('#actual_effort_"+ numRows + "').html()) ;if(this.value==100) updatecost("+ numRows + ");SUGAR.grid.updateAncestorsPercentComplete("+ numRows + ");SUGAR.grid.save("+ numRows + ");SUGAR.gantt.changeTask("+ numRows + ");\" >" +
 							 "<option value='0'>Not started</option><option value='50'>In Progress</option><option value='100'>Completed</option></select><input type='hidden' name='percent_complete_" + numRows +  "' id='percent_complete_" + numRows +  "' value=''>";
			cell.setAttribute('class', 'PM CS CD color1');
						
			// Cost - calculed
			cell = row.insertCell(9);
			cell.innerHTML = "<div style='display:inline;'></div>";
			cell.setAttribute('class', 'PM color3');
			...
			cell.innerHTML =
				"<input class='startdate' name='date_start_" + numRows + "' id='date_start_" + numRows + "' style='border:0' onclick='javascript:current_id=" + numRows +  "' onchange='parseDate(this, \"" + calendar_dateformat + "\");' value=''" +
					" type='text' tabindex='2' size='11' maxlength='10' value=''>";
			cell.setAttribute('class', 'PM color3');
					
			// Start Time
			cell = row.insertCell(13);
			cell.innerHTML ="<input class='time' name='time_start_" + numRows + "' id='time_start_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='09:00'>";
			cell.setAttribute('class', 'PM color3');
			
			// finish Date
			cell = row.insertCell(14);
			cell.innerHTML =
				"<input class='finishdate' name='date_finish_" + numRows + "' id='date_finish_" + numRows + "' style='border:0' onclick='javascript:current_id=" + numRows +  "' onchange='parseDate(this, \"" + calendar_dateformat + "\");' " +
					" type='text' tabindex='2' size='11' maxlength='10' value=''>";
			cell.setAttribute('class', 'PM color3');

			// End Time
			cell = row.insertCell(15);
			cell.innerHTML ="<input class='time' name='time_finish_" + numRows + "' id='time_finish_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='17:00'>";
			cell.setAttribute('class', 'PM color3');
			...
			document.getElementById('duration_unit_hidden_' + numRows).value = document.getElementById('duration_unit_' + numRows)[document.getElementById('duration_unit_' + numRows).selectedIndex].value;
			...
			cell.innerHTML =
				"<input type=text  style='border:0' size=10 name=predecessors_" + numRows + " id=predecessors_" + numRows +
					" onBlur='if (SUGAR.grid.validatePredecessors(this)){" +
						...
						"SUGAR.grid.updateAllDependentsAfterDateChanges(" + numRows + ");" +
						"SUGAR.gantt.changeTask(" + numRows + ");" +
						"SUGAR.grid.save(" + numRows + ");}}" +
						"SUGAR.grid.dependencyCheckFail = 0;'>";
			cell.setAttribute('class', 'PM color3');

			// initial Cost - calculed
			cell = row.insertCell(17);
			cell.innerHTML ="<input class='initial_cost_gbp small' readonly='readonly' type='text' size='10' style='border:0' name='initial_cost_gbp_" + numRows + "' id='initial_cost_gbp_" + numRows + "' value=''>";
			cell.setAttribute('class', 'CS color5');
			cell.setAttribute('style', 'display:none');
			
			// revised Cost - calculed
			cell = row.insertCell(18);
			cell.innerHTML ="<input class='revised_cost_gbp small' readonly='readonly' type='text' size='10' style='border:0' name='revised_cost_gbp_" + numRows + "' id='revised_cost_gbp_" + numRows + "' value=''>";
			cell.setAttribute('class', 'CS color5');
			cell.setAttribute('style', 'display:none');

			// final Cost - calculed
			cell = row.insertCell(19);
			cell.innerHTML ="<input class='final_cost_gbp small' readonly='readonly' type='text' size='10' style='border:0' name='final_cost_gbp_" + numRows + "' id='final_cost_gbp_" + numRows + "' value=''>";
			cell.setAttribute('class', 'CS color5');
			cell.setAttribute('style', 'display:none');

			// Variance Initial Final - calculeted
			cell = row.insertCell(20);
			cell.innerHTML ="<input class='variance_initial_final_gbp small' readonly='readonly' type='text' size='10' style='border:0' name='variance_initial_final_gbp_" + numRows + "' id='variance_initial_final_gbp_" + numRows + "' value=''>";
			cell.setAttribute('class', 'CS color5');
			cell.setAttribute('style', 'display:none');

			// Variance Revised Final - calculed 
			cell = row.insertCell(21);
			cell.innerHTML ="<input class='variance_revised_final_gbp small' readonly='readonly' type='text' size='10' style='border:0' name='variance_revised_final_gbp_" + numRows + "' id='variance_revised_final_gbp_" + numRows + "' value=''>";
			cell.setAttribute('class', 'CS color5');
			cell.setAttribute('style', 'display:none');

			// PIR - calculed  - calculed 
			cell = row.insertCell(22);
			cell.innerHTML ="<div id='pir_calculed_" + numRows + "' style='display:inline;'></div>";
			cell.setAttribute('class', 'CS color5');
			cell.setAttribute('style', 'display:none');

			// Payment date - calculed
			cell = row.insertCell(23);
			cell.innerHTML ="<div id='payment_date_" + numRows + "' style='display:inline;'></div>";
			cell.setAttribute('class', 'CS color5');
			cell.setAttribute('style', 'display:none');
			...
			cell.innerHTML ="<input class='small' name='estimated_effort_" + numRows + "' id='estimated_effort_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:temp=this.value;' onkeyup='javascript:temp = checkAmount(this, temp, 5, 2, false);updatecost(" + numRows + ");SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// Revised<br/>Estimated<br/>Quantity
			cell = row.insertCell(25);
			cell.innerHTML ="<input class='small' name='revised_effort_" + numRows + "' id='revised_effort_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:temp=this.value;' onkeyup='javascript:temp = checkAmount(this, temp, 5, 2, false);updatecost(" + numRows + ");SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// Final<br/>Estimated<br/>Quantity
			cell = row.insertCell(26);
			cell.innerHTML ="<input class='small' name='final_effort_" + numRows + "' id='final_effort_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:temp=this.value;' onkeyup='javascript:temp = checkAmount(this, temp, 5, 2, false);updatecost(" + numRows + ");SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');
			
			// Timesheet - calculed
			cell = row.insertCell(27);
			cell.innerHTML ="<div id='actual_effort_' style='display:inline;'></div>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// Units
			cell = row.insertCell(28);
			cell.innerHTML ="<input class='small unit' name='unit_" + numRows + "' id='unit_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onkeyup='javascript:SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// COST per unit
			cell = row.insertCell(29);
			cell.innerHTML ="<input class='small' name='cost_unit_" + numRows + "' id='cost_unit_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onkeyup='javascript:temp = checkAmount(this, temp, 5, 2, false);SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');
	
			// Currency
			cell = row.insertCell(30);
			cell.innerHTML ="<input class='currency unit' name='currency_" + numRows + "' id='currency_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:current_id=" + numRows + "' onkeyup='javascript:SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// Rate
			cell = row.insertCell(31);
			cell.innerHTML ="<input class='small' name='rate_" + numRows + "' id='rate_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:temp=this.value;' onkeyup='javascript:temp = checkAmount(this, temp, 5, 2, false);SUGAR.grid.save();'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');
					
			// %Initial Cost Currency - calculed
			cell = row.insertCell(32);
			cell.innerHTML ="<input class='initial_cost small' readonly='readonly' name='initial_cost_" + numRows + "' id='initial_cost_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value=''>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// Revised Cost Currency - calculed but editable
			cell = row.insertCell(33);
			cell.innerHTML ="<input class='revised_cost small' name='revised_cost_" + numRows + "' id='revised_cost_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:current_id=" + numRows + ";temp=this.value;' onkeyup='javascript:temp = checkAmount(this, temp, 5, 2, false);SUGAR.grid.save(" + numRows + ");'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// Final Cost Currency - calculed but editable
			cell = row.insertCell(34);
			cell.innerHTML ="<input class='final_cost small' name='final_cost_" + numRows + "' id='final_cost_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:current_id=" + numRows + ";temp=this.value;' onkeyup='javascript:temp = checkAmount(this, temp, 5, 2, false);SUGAR.grid.save(" + numRows + ");'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// Supplier Invoice number
			cell = row.insertCell(35);
			cell.innerHTML ="<input class='small' name='supplier_invoice_number_" + numRows + "' id='supplier_invoice_number_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:current_id=" + numRows + ";temp=this.value;' onkeyup='javascript:temp = checkAmount(this, temp, 10, 0, false);SUGAR.grid.save(" + numRows + ");'>";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');

			// PIR
			cell = row.insertCell(36);
			cell.innerHTML ="<input class='small' name='pir_" + numRows + "' id='pir_" + numRows + "' style='border:0'	type='text' tabindex='2' size='7' maxlength='7' value='' onclick='javascript:current_id=" + numRows + ";temp=this.value;' onkeyup=\"javascript:temp = checkAmount(this, temp, 10, 0, false);$('#pir_calculed_" + numRows + "').html(this.value);SUGAR.grid.save(" + numRows + ");\">";
			cell.setAttribute('class', 'CD color7');
			cell.setAttribute('style', 'display:none');
...
/*
		...
		selectRow: function(mappedRow)
		{
			...
			$('#project_task_row_' + mappedRow).addClass("sqsSelectedSmartInputItem");
		...
		unSelectRow: function(mappedRow)
		{
			...
			if (index >= 0) {
				...
				$('#project_task_row_' + mappedRow).removeClass("sqsSelectedSmartInputItem");
			}
		},
		...
		calculateDatesAfterAddingPredecessors: function(row) {
			if (SUGAR.grid.isLoaded){
				...
				if (predecessors[row] != null && predecessors[row].length > 0) {
					...
					document.getElementById('date_start_' + row).value = SUGAR.grid.getDisplayDate(startDateObj.date);;
...
//			extendedAncestors.sort(SUGAR.grid.sortNumber);
		...
		*/
		...
		// dac
		newCalculateDurationHours: function(dateStart, dateFinish, row) {
			var startDate = SUGAR.grid.getJSDate(dateStart);
			var startTime = document.getElementById("time_start_" + row).value
			var endDate = SUGAR.grid.getJSDate(dateFinish);
			var endTime = document.getElementById("time_finish_" + row).value
			...
			$("#duration_" + row).val(duration);
			
		},
		
		
		//dac
		newCalculateDuration: function(dateStart, dateFinish, row) {
		
			if (document.getElementById("duration_unit_" + row).value == "Hours")
				return SUGAR.grid.newCalculateDurationHours(dateStart, dateFinish, row);
		...
		// dac
		newProcessStartDate: function(row){
			SUGAR.grid.newCalculateDuration($("#date_start_" + row).val(), $("#date_finish_" + row).val(), row);
			SUGAR.gantt.changeTask(row);
		},
		
		// dac
		newProcessFinishDate: function(row){
			SUGAR.grid.newCalculateDuration($("#date_start_" + row).val(), $("#date_finish_" + row).val(), row);
			SUGAR.gantt.changeTask(row);
		},
		...
		processStartDate: function(obj, row){
			if (SUGAR.grid.validateDate(obj) && SUGAR.grid.validateEndDateAfterStartDate(row)){
				...
				SUGAR.grid.calculateDuration(document.getElementById("date_start_" + row).value, document.getElementById("date_finish_" + row).value, row);
				SUGAR.grid.updateAllDependentsAfterDateChanges(row);
				//SUGAR.grid.updateAncestorsTimeData(row);
				SUGAR.gantt.changeTask(row);
			}
...
//			debugger
			...
			if (document.getElementById('selected_view') &&
		        ...
		        var aMainMenuItems = new Array
		                (SUGAR.language.get('Project', 'LBL_INSERT_BUTTON'),
		                 ...
		                 // DAC 3-11-2009
		                 SUGAR.language.get('Project', 'LBL_VIEW_SESSION_BUTTON'));
        ...
        onContextMenuItemClick: function(p_sType, p_aArguments, p_oItem) {
            ...
            switch(this.index) {
				...
				case 14: 	// View Task Details
					...
					break;
				case 15: 	// View Session Details
					SUGAR.grid.viewSessionDetails();
					break;
					/*
		...
		updateResourceFullNameAndType: function(row) {
			document.getElementById("resource_full_name_" + row).value = document.getElementById("resource_" + row).options[document.getElementById("resource_" + row).selectedIndex].innerHTML;
		...
		updateResourceUsed: function(row) {
			if ($('#resource_type_' + row ).val() == 'User') 	$('#resource_used_' + row).val('INT');
			if ($('#resource_type_' + row ).val() == 'Account') $('#resource_used_' + row).val('EXT');
			if ($('#resource_type_' + row ).val() == 'Contact') $('#resource_used_' + row).val('EXT');
		},
		...
		/**
		 * Dac
		 * vieSessionDetails: this function loads the hidden iframe with the task details.
		 */
		viewSessionDetails: function () {
			if (selectedRows.length == 0)
				return;
			var row = selectedRows[0];
			var record = document.getElementById("obj_id_" + row).value;
			document.getElementById("task_detail_area_div").style.display = "";
			frames["task_detail_area_iframe"].location.href = "index.php?module=Meetings&fromGrid=1&show_js=1&action=EditView&parent_module=ProjectTask&return_module=ProjectTask&return_action=DetailView&name=test&parent_id=2d5e9e73-72f8-aebf-8886-4ae1ade8f611&parent_name=test&parent_type=ProjectTask&relate_id2d5e9e73-72f8-aebf-8886-4ae1ade8f611&relate_to=ProjectTask&record" + record;
			window.setTimeout('SUGAR.grid.loadTaskDetailsDiv()', 2000);
		...
		closeTaskDetails: function(){
			...
			document.getElementById("task_detail_area_div").innerHTML = '<img src="themes/default/images/img_loading.gif" border="0" />';
		},
		...
		save: function(id) {
			$('#numRowToSave').val(id);
			// only saveif project task id k
			if (($('#obj_id_'+id).val() != '') || ($('#obj_id_'+id).val() == '' && $('#save_in_progress_'+id).val() == '')){
				$('#save_in_progress_'+id).val('yes');
