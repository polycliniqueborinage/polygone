...
SUGAR.gantt = function () {
	return {
		...
		compareToToday: function (date) {
			var today = new Date();
			if ((today.getMonth() == date.getMonth()) && (today.getDate() == date.getDate()) && (today.getFullYear() == date.getFullYear())) {
				return true;
			}
			return false;
		},
		...
		getTaskRowMap: function () {
			...
			for (var i = 1; i < taskRows.length; i++) {
				...
				rowMap[i - 1] = {
					id: SUGAR.grid.getActualRow(rowId)
				};
			}
			return rowMap;
		},
		createTable: function (view_type, calendar_start, bgcolor) {
			...
			ganttDates.setAttribute('height', '30');
			...
			ganttDateEmptyFirstCell.innerHTML = "<a href='#' onclick='SUGAR.gantt.moveChart(\"backward\", document.getElementById(\"gantt_chart_start_date\").value, \"" + bgcolor + "\");'><img width='8' height='11' border='0' align='absmiddle' alt='" + SUGAR.language.get('Project', 'LBL_LAST_WEEK') + "' src='themes/Sugar/images/previous.gif'/></a>";
			...
			ganttDateEmptyLastCell.innerHTML = "<a href='#' onclick='SUGAR.gantt.moveChart(\"forward\", document.getElementById(\"gantt_chart_start_date\").value, \"" + bgcolor + "\");'><img width='8' height='11' border='0' align='absmiddle' alt='" + SUGAR.language.get('Project', 'LBL_NEXT_WEEK') + "' src='themes/Sugar/images/next.gif'/></a>";
			...
			// dac
			if (rowMap.length >1 ) {
				for (var i = 0; i < rowMap.length-1; i++) {
		...
		changeTask: function (task_num) {
			if (!document.getElementById("gantt")) return;
			oneMinute = 1000 * 60;
			var oneDay = oneMinute * 60 * 24;
			var oneWeek = oneDay * 7;
			...
			if (!emptyRow) {
				...
				bar.onmouseover = function () {
					SUGAR.gantt.taskOverLib(SUGAR.gantt.popupInfo(task_num), document.getElementById("description_" + task_num).value);
				}
		...
		PMOnly: function () {
			...
			Set_Cookie("project_management_view", "pm_only", 3000, false, false, false);
			$('.CS').hide();
 			$('.CD').hide();
			$('.PM').show();
		...
		CSOnly: function () {
			...
			resizer.style.visibility = "hidden";
			grid.style.width = "100%";
			Set_Cookie("project_management_view", "cs_only", 3000, false, false, false);
			$('.PM').hide();
			$('.CD').hide();
 			$('.CS').show();
 			SUGAR.grid.showProjectButtons("grid_only");
		...
		CDOnly: function () {
			...
			resizer.style.visibility = "hidden";
			grid.style.width = "100%";
			Set_Cookie("project_management_view", "cd_only", 3000, false, false, false);
			$('.PM').hide();
			$('.CS').hide();
 			$('.CD').show();
 			SUGAR.grid.showProjectButtons("grid_only");
		...
		sliderInit: function () {
			...
			horizontalSlider.onChange = function (offsetFromStart) {
				if (offsetFromStart == 25) SUGAR.gantt.createTable('week', document.getElementById('calendar_start').value);
				else if (offsetFromStart == 50) SUGAR.gantt.createTable('biweek', document.getElementById('calendar_start').value);
				else if (offsetFromStart == 75) SUGAR.gantt.createTable('month', document.getElementById('calendar_start').value);
		...
		setupCalendar: function (dateformat, bgcolor) {
			...
			bg_color = bgcolor;
			Calendar.setup({
				inputField: "calendar_start",
				ifFormat: calendar_dateformat,
				showsTime: false,
				button: "calendar_start",
				singleClick: true,
				step: 1
			});
		}
...
function updatecost(id) {
	...
	estimated_effort 	= parseFloat($('#estimated_effort_'+id).val());
	revised_effort 		= parseFloat($('#revised_effort_'+id).val());
	final_effort 		= parseFloat($('#final_effort_'+id).val());
	cost_unit 			= parseFloat($('#cost_unit_'+id).val());
	rate 				= parseFloat($('#rate_'+id).val());
	
	if (id!='') {
	
		initial_cost = roundNumber(estimated_effort * cost_unit,2);
		revised_cost = roundNumber(revised_effort * cost_unit,2);
		final_cost = roundNumber(final_effort * cost_unit,2);

		initial_cost_gbp = roundNumber(estimated_effort * cost_unit * rate,2);
		revised_cost_gbp = roundNumber(revised_effort * cost_unit * rate,2);
		final_cost_gbp = roundNumber(final_effort * cost_unit * rate,2);

		variance_initial_final_gbp = roundNumber(final_cost_gbp - initial_cost_gbp,2);
		variance_revised_final_gbp = roundNumber(final_cost_gbp - revised_cost_gbp,2);
		
		if (isNaN(initial_cost)) { $('#initial_cost_'+id).val('?');} else {$('#initial_cost_'+id).val(initial_cost)}
		if (isNaN(revised_cost)) { $('#revised_cost_'+id).val('?');} else {$('#revised_cost_'+id).val(revised_cost);}
		if (isNaN(final_cost)) 	 { $('#final_cost_'+id).val('?');} else {$('#final_cost_'+id).val(final_cost);}

		if (isNaN(initial_cost_gbp)) { $('#initial_cost_gbp_'+id).val('?');} else {$('#initial_cost_gbp_'+id).val(initial_cost_gbp)}
		if (isNaN(revised_cost_gbp)) { $('#revised_cost_gbp_'+id).val('?');} else {$('#revised_cost_gbp_'+id).val(revised_cost_gbp);}
		if (isNaN(final_cost_gbp)) 	 { $('#final_cost_gbp_'+id).val('?');} else {$('#final_cost_gbp_'+id).val(final_cost_gbp);}
		
		if (isNaN(variance_initial_final_gbp)) { $('#variance_initial_final_gbp_'+id).val('?');} else {$('#variance_initial_final_gbp_'+id).val(variance_initial_final_gbp)}
		if (isNaN(variance_revised_final_gbp)) { $('#variance_revised_final_gbp_'+id).val('?');} else {$('#variance_revised_final_gbp_'+id).val(variance_revised_final_gbp);}

		// sum
		initial_cost_sum			= 0;
		revised_cost_sum			= 0;
		final_cost_sum				= 0;
		initial_cost_gbp_sum		= 0;
		revised_cost_gbp_sum		= 0;
		final_cost_gbp_sum			= 0;
		variance_initial_final_sum	= 0;
		variance_revised_final_sum	= 0;

		$(".initial_cost").each(function(){
			if (isNaN(this.value)) { } else { initial_cost_sum = initial_cost_sum + parseValue(this.value); }
		});

		$(".revised_cost").each(function(){
			if (isNaN(this.value)) { } else { revised_cost_sum = revised_cost_sum + parseValue(this.value); }
		});
	
		$(".final_cost").each(function(){
			if (isNaN(this.value)) { } else { final_cost_sum = final_cost_sum + parseValue(this.value); }
		});

		$(".initial_cost_gbp").each(function(){
			if (isNaN(this.value)) { } else { initial_cost_gbp_sum = initial_cost_gbp_sum + parseValue(this.value); }
		});

		$(".revised_cost_gbp").each(function(){
			if (isNaN(this.value)) { } else { revised_cost_gbp_sum = revised_cost_gbp_sum + parseValue(this.value); }
		});

		$(".final_cost_gbp").each(function(){
			if (isNaN(this.value)) { } else { final_cost_gbp_sum = final_cost_gbp_sum + parseValue(this.value); }
		});

		$(".variance_initial_final_gbp").each(function(){
			if (isNaN(this.value)) { } else { variance_initial_final_sum = variance_initial_final_sum + parseValue(this.value); }
		});

		$(".variance_revised_final_gbp").each(function(){
			if (isNaN(this.value)) { } else { variance_revised_final_sum = variance_revised_final_sum + parseValue(this.value); }
		});

		$("#initial_cost_sum").html(roundNumber(initial_cost_sum,2));
		$("#revised_cost_sum").html(roundNumber(revised_cost_sum,2));
		$("#final_cost_sum").html(roundNumber(final_cost_sum,2));

		$("#initial_cost_gbp_sum").html(roundNumber(initial_cost_gbp_sum,2));
		$("#revised_cost_gbp_sum").html(roundNumber(revised_cost_gbp_sum,2));
		$("#final_cost_gbp_sum").html(roundNumber(final_cost_gbp_sum,2));

		$("#variance_initial_final_sum").html(roundNumber(variance_initial_final_sum,2));
		$("#variance_revised_final_sum").html(roundNumber(variance_revised_final_sum,2));
	
	}

}

function updateNextTaskStartDate(task) {
	
	date_finish =  $('#date_finish_'+task).val() ;
	time_finish =  $('#time_finish_'+task).val() ;
	
	$('.predecessors').each(function(){ // Use each to gain access to each element as $(this)
   		if ( $(this).val() == task ) { 
   			$('#date_start_'+$(this).attr('id').substr( $(this).attr('id').lastIndexOf("_")+1)).val(date_finish);
   			$('#time_start_'+$(this).attr('id').substr( $(this).attr('id').lastIndexOf("_")+1)).val(time_finish);
   		}
	});
