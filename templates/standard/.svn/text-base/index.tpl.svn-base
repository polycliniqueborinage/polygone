{include file="header.tpl" js_jquery132="yes" js_jquery_ui_171="yes" js_ajax="yes" js_jquery_datepicker="yes" js_jqmodal="yes" js_agenda="yes"}
					...
					{/if}
						<tr {if $tasks[task].daysleft < 0} class="marker-late"{elseif $tasks[task].daysleft == 0} class="marker-today"{/if}>
							...
							<td class="tools">
								...
								<a class="tool_edit" href="managetask.php?action=editform&amp;tid={$tasks[task].ID}&amp;id={$tasks[task].project}" title="{#edit#}"></a>{/if}
	...
	<div class="msgs">
			...
			<div class="block" id="activityhead" style = "{$actstyle}">
				<table cellpadding="0" cellspacing="0" border="0">
					...
					{/if}
						<tr>
							<td>{if  $userpermissions.messages.close}<a class="butn_reply" href="managemessage.php?action=replyform&amp;mid={$messages[message].ID}&amp;id={$messages[message].project}" title="{#answer#}"></a>{/if}</td>
							<td>
								<div class="toggle-in">
								<span class="acc-toggle" onclick="javascript:accord_msgs.activate($$('#activityhead .accordion_toggle')[{$smarty.section.message.index}]);toggleAccordeon('activityhead',this);"></span>
									<a href="managemessage.php?action=showmessage&amp;mid={$messages[message].ID}&amp;id={$messages[message].project}" title="{$messages[message].title}">
										{$messages[message].title|truncate:30:"...":true}
									</a>
								</div>
							</td>
							<td><a href = "managemessage.php?action=showproject&amp;id={$messages[message].project}">{$messages[message].pname|truncate:20:"...":true}</a></td>
															...
															<li>
																<div class="itemwrapper" id="iw_{$messages[message].files[file].ID}">
																		...
																		<table cellpadding="0" cellspacing="0" border="0">
																			...
																			<tr>
																				<td colspan="3"><span class="name"><a href = "{$messages[message].files[file].datei}" {if $messages[message].files[file].imgfile == 1} rel="lytebox[]" {elseif $messages[message].files[file].imgfile == 2} rel = "lyteframe[text]" {/if} title="{$messages[message].files[file].name}">{$messages[message].files[file].name|truncate:15:"...":true}</a></span>	</td>
																			<tr/>
																		</table>
